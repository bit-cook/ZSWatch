...
{
    LOG_INF("Reading %d bytes from file", size);
    if (IS_SPECIAL_FULL_FS_FILE(zfp->filep)) {
        int rc;
        ...
        LOG_INF("Res: %d, bytes read: %d", res, bytes_read);
        if (res != LV_FS_RES_OK) {
            return -EIO;
        }
...
{
    int rc;
    LOG_INF("Seeking %d bytes in file, whence: %d", (int)offset, whence);
...
static lv_fs_res_t lvgl_fs_tell(struct _lv_fs_drv_t *drv, void *file, uint32_t *pos_p)
{
    ...
    *pos_p = open_file->index;
    LOG_INF("Telling file, pos: %d", *pos_p);
    return LV_FS_RES_OK;
}
    ...
    const char *path, struct fs_dirent *entry)
{
    ...
    LOG_INF("stat file %s", file_name_ptr);
...
static int zsw_fs_unlink(struct fs_mount_t *mountp, const char *path)
{
    ...
    LOG_INF("unlink file %s", file_name_ptr);
