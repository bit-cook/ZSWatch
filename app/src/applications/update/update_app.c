                                                  ...
                                                  void *data, size_t data_size)
{
    ...
    if (!dfu_in_progress) {
        ...
        LOG_INF("DFU started: Image %u, Offset %u, Size %llu", img_data->req->image, img_data->req->off, img_data->action->size);
        char status_msg[64];
        snprintf(status_msg, sizeof(status_msg), "%s", state.payload);
        update_ui_set_status(status_msg);
        
    } else {
        LOG_INF("DFU in progress: Image %u, Offset %u, Size %llu", img_data->req->image, img_data->req->off, img_data->action->size);
    }
